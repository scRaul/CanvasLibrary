...
class Screen {
  ...
  drawPoint(povar, color) {
    if(!povar instanceof Vec ) return;
    var p = vMath(this.#screenBasis, '*', povar);
    this.#setPixelColor(p.x, p.y, color);
  }
  drawBSpline(povars,color,pen=null){
    var basis = getMat4(1,-3,3,-1,
      4,0,-6,3,
      1,3,3,-3,
      0,0,0,1);
    basis = vMath( (1/6.0), '*', basis );
    for(let i = 0; i < povars.length-3; i ++){
      var  cntrlP = getMat4C(povars[i],povars[i+1],povars[i+2],povars[i+3]);
