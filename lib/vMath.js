//Very simple vector math library 
// vector objct 
class Vec{
    x=0;y=0;z=0;w=0;
    constructor(x=0,y=0,z=0,w=0){ this.x = x;this.y = y;this.z = z;}
    normalized(){
        return vMath(this,'*',1/this.magnitude());
    }
    magnitude(){
        return Math.sqrt(this.x * this.x + this.y * this.y + this.z * this.z + this.w * this.w );
    }
    copy(){ return new Vec(this.x,this.y,this.z,this.w); }
}
// matrix object 
class Mat{
    x1=0; y1=0; z1=0;w1=0;
...
// construct a 4x4 matrix from columns aka vec objects
...
// helper function to do math with Matrixes and Vec 
...
function vswap(list,a,b){
    let temp = list[a];
    list[a] = list[b];
    list[b] = temp;
}
// QUICK SORTING Vecs by Magnitude  and direction
function vpart(list,left,right){
    let i = left-1;
    for(let j = left; j < right; j++){
        if(list[j].magnitude()*list[j].normalized() < list[right].magnitude() * list[j].normalized()){
            i++;
            vswap(list,i,j);
        }
    }
    vswap(list,i+1,right); 
    return i+1;
}
//axes = 0 for x, 1 for y
function vquickSort(list,left,right){
    if(left >= right) return;
    let pivot;
    pivot = vpart(list,left,right);
    vquickSort(list,left,pivot-1);
    vquickSort(list,pivot+1,right);
}

// Helper function for determining if point lies inside of a function 
function sign (p1,p2,p3)
{
    return (p1.x - p3.x) * (p2.y - p3.y) - (p2.x - p3.x) * (p1.y - p3.y);
}
//returns true if pt inside v1,v2,v3 or false if not 
